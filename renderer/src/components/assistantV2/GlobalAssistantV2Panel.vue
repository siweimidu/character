...
function startProviderSpeechRecognition(): void {
  ...
  speechRecorder = createSpeechRecorder(async (audioData, mimeType) => {
    ...
    try {
      const res = await window.characterArc.transcribeSpeech({ settings: toPlainSpeechSettings(settings), audioData: uint8ArrayToBase64(audioData), audioType: mimeType })
...
/** 重新生成：透传与正常发送一致的智能体选择与模式意图，避免回落默认配置。 */
async function handleRegenerateTurn(turnId: string): Promise<void> {
  await assistant.regenerateTurn(turnId, {
    intentHint: 'global-assistant-v2:chat',
    agentId: selectedAgentId.value || undefined,
    agentScope: selectedAgentScope.value
  })
  ...
  <section class="v2-dock">
    ...
    <div v-else class="chat-pane">
      <AssistantMessages
        ...
        @undo="handleUndoTurn"
        @regenerate="handleRegenerateTurn"
