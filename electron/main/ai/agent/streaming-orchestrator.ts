...
export async function runStreamingAgentTask(
  ...
  const moduleLabels: Record<string, string> = {
    ...
    plotThreads: '伏笔线索（使用 read_project_data entity_type=plot_threads）',
  ...
  const chapterToolsBlock = [
    ...
    '- `read_project_data`: 读取项目设定与资料，支持先取索引，再按 `entity_id` / `summary_only` / `limit` / `doc_key` 精读世界观、角色、组织、组织成员、关系、大纲、章节、伏笔线索、灵感、项目知识、公共拆书库、参考书、创作记忆、项目约束',
    ...
    '- `search_project`: 搜索项目中的世界观、角色、组织、组织成员、关系、大纲、章节、伏笔线索、灵感、项目知识、公共拆书库、参考书、创作记忆、项目约束等资料，并返回 `entity_type` / `entity_id`',
